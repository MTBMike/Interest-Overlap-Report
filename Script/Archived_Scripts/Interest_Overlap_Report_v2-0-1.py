...
import arcpy, time, ntpath, datetime, itertools, win32com.client, math
...
# Add logo to report and set size and location
pic = sheet.Pictures().Insert(r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\ArcGIS_Tools\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\Image\Overlap_Example.JPG")
...
#            for shrow in arcpy.SearchCursor(arcpy.MakeFeatureLayer_management(clip_feature, "clipAOI", sqlQuery)):
            ...
            for shrow in arcpy.SearchCursor(extentFile):
                arcpy.AddMessage(str(shrow.getValue(str(shField))))
...
arcpy.AddMessage(mineral_Coal_first)
arcpy.AddMessage(layerListDict)
...
if mapRequest == True:
    ...
    def addOLEObject_to_Excel(sheet2,pdf):
...
# Commented this out to remove option to add pdf'd map to excel spreassheet. The issue is that the map needs editing before a final product
# can be sent to the end user. Will need to script more advanced functionaity to create a 'finalized' map for use in the report.
# For now, the script will produce an mxd and the user can make some fin adjustments afterwards.    
    
##    pdf = output_folder + "\\" + output_name + "_" + strftime('%d%b%Y') + ".pdf"
##
##    arcpy.mapping.ExportToPDF(mxd, pdf)
##
##    addOLEObject_to_Excel(sheet2, pdf) 
