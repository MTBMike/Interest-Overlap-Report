...
import arcpy, time, ntpath, datetime, itertools, win32com.client, math, os
...
mapRequest = arcpy.GetParameter(8)
orientation = arcpy.GetParameterAsText(9)
mapTitle = arcpy.GetParameterAsText(10)
mapSubtitle = arcpy.GetParameterAsText(11)


## Hard coded versions of parameters
#clip_feature = r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\ArcGIS_Tools\Testing_Area\mmacrae\Geodatabases\Clipped_FeatureClasses.gdb\DistrictTest_Grids"
#sqlQuery = "OBJECTID = 1"
#shFieldList = ""
#xlsOption = "MEMPRD"
...
## 'Integrated Cadastral Fabric Restricted Access',
...
clippedGDB = "IOR_Clipped_FeatureClasses_" + output_name + "_" + strftime('%d%b%Y') + ".gdb"
...
pic = sheet4.Pictures().Insert(r"\\zipline\S6203\rddTB.shr\Mineral-Titles-Br\Admin\logos\approved_logos\BCID+ENER\2013_BC_ENER\ENER\BC_ENER_H_CMYK_pos.jpg")
...
sheet4.Cells(sumexcelrow + 1,sumexcelcol + 2).Value = "Report run on " + strftime('%d%b%y') + " @ " + strftime('%I:%M:%S')
    ...
    # End of conditional sentence. If the user chose some fields from the area of interest, update the header
        ...
        for shField in shFieldList:
            ...
            for shrow in arcpy.SearchCursor(extentFile):
                arcpy.AddMessage(str(shrow.getValue(shField)))
...
arcpy.AddMessage("FieldList = " + str(fieldList))
...
#                arcpy.AddMessage("Before making feature layer")
...
#                arcpy.AddMessage("After making feature layer and before selecting")
                ...
                arcpy.AddMessage(selFeats)
#                arcpy.AddMessage("After selecting")
...
#                        print row[d["Featureclass_Name"]] + " = " + str(countFeats)
                        countFeatsDict[row[d["Featureclass_Name"]]] = countFeats
                        
                        print "Dictionary = " + str(countFeatsDict)
                ...
                # Also, do some formatting on the cells
                    ...
                    arcpy.AddMessage("AddMessage = " + outClipFC)
                    ...
                    if row[d["Featureclass_Name"]] in ['Tenure - Coal License Applications',
                        ...
                        print row[d["Featureclass_Name"]] + " = " + str(countClippedFeats)
...
for locale in sorted(locationList):
    for row in arcpy.SearchCursor(xls,'Featureclass_Name = "{0}"'.format(locale)):
        ...
        for rowLocale in arcpy.SearchCursor("locale"):
            #print str(rowLocale.getValue(str(row.getValue('Fields_to_Summarize'))))
...
if mapRequest == True:
    ...
    def addOLEObject_to_Excel(sheet5,pdf):
    ...
    # Set second sheet in book and rename it for the parameters
    book.Sheets.Add()
    sheet5 = book.Worksheets('Sheet5')
    sheet5.Name = "Map"
    sheet5.Move(After=book.Worksheets("Districts and BCGS-NTS Location"))    
    
    arcpy.AddMessage('Create Map')
    if orientation == 'Landscape':
        arcpy.AddMessage(orientation)
        mxd = arcpy.mapping.MapDocument(r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\Templates\mxd_templates\2014_Templates\Template_(8.5x11Landscape)_26Jun14.mxd")
        titlex = 24.7749
        titley = 17.7057
        subtitlex = 24.7749
        subtitley = 16.8424
        ...
        arcpy.AddMessage(orientation)
        mxd = arcpy.mapping.MapDocument(r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\Templates\mxd_templates\2014_Templates\Template_(8.5x11Portrait)_26Jun14.mxd")
        titlex = 3.2244
        titley = 2.0283
        subtitlex = 3.2244
        subtitley = 1.2077
    ...
    for elm in arcpy.mapping.ListLayoutElements(mxd):
        ...
        if elm.name == 'SUBTITLE':
            if mapSubtitle == '':
                ...
                elm.text = mapSubtitle.upper()
                elm.elementPositionX = subtitlex
                elm.elementPositionY = subtitley
                                                                      
    layerList = []
    
    arcpy.AddMessage(output_workspace)
    ...
    for fc in arcpy.ListFeatureClasses():
        if fc == 'extentFile':
            ...
            arcpy.ApplySymbologyFromLayer_management (Layer, r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\ArcGIS_Tools\Testing_Area\mmacrae\Layer_Files\extentFile.lyr")
            addLayer = arcpy.mapping.AddLayer(df, Layer)
    ...
    if df.scale < 5000:
        df.scale = 5000
    elif df.scale >= 5001 and df.scale <= 25000:
        df.scale = 25000
    elif df.scale >= 25001 and df.scale <= 50000:
        df.scale = int(math.ceil(df.scale/10000)*10000)
    elif df.scale >= 50001 and df.scale <= 100000:
        df.scale = int(math.ceil(df.scale/10000)*10000)
    elif df.scale > 100000:
        df.scale = int(math.ceil(df.scale/25000)*25000)

    
    mxd.saveACopy(output_folder + "\\" + output_name + "_" + strftime('%d%b%Y') + ".mxd")
    

    arcpy.AddMessage('Map Saved')
    
    pdf = output_folder + "\\" + output_name + "_" + strftime('%d%b%Y') + ".pdf"

    arcpy.mapping.ExportToPDF(mxd, pdf)

    addOLEObject_to_Excel(sheet5, pdf) 
...
arcpy.AddMessage(output_folder + "\\" + "Interest_report_" + output_name + "_" + strftime('%d%b%Y') + ".xlsx")
