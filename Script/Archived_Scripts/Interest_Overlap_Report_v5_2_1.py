...
def getLayerInfo(lyrDict, xlsRow, overlap):
    ...
    if overlap == True:
        if xlsRow[1] in lyrDict:
            lyrDict[xlsRow[1]].update({xlsRow[2]: [str(xlsRow[0] + "_clip"), str(xlsRow[26]), str(xlsRow[34]), str(xlsRow[35])]})
        else:
            lyrDict[xlsRow[1]] = {}
            lyrDict[xlsRow[1]].update({xlsRow[2]: [str(xlsRow[0] + "_clip"), str(xlsRow[26]), str(xlsRow[34]), str(xlsRow[35])]})
        ...
        if xlsRow[1] in lyrDict:
            lyrDict[xlsRow[1]].update({xlsRow[2]: ["No Overlap Found", str(xlsRow[26]), str(xlsRow[34]), str(xlsRow[35])]})
        else:
            lyrDict[xlsRow[1]] = {}
            lyrDict[xlsRow[1]].update({xlsRow[2]: ["No Overlap Found", str(xlsRow[26]), str(xlsRow[34]), str(xlsRow[35])]})
...
def processData(processedAOI, processedAOI_Hectares, IOR_Data, IORData_Fields):
    ''' 
...
def createInterestReportSheet(book, layerListDict, appsDict):
    '''
...
def createSummarySheet(book, excel, processedAOI, processedAOI_Hectares, collectFeatsCountDict, iMapBCBaseURL, crossReferenceDict, geoMark_URL):
    ...
    if os.path.basename(processedAOI) not in [r'MTA_SPATIAL.MTA_ACQUIRED_TENURE_SVW', r'MTA_SPATIAL.MTA_SITE_SVW', r'WHSE_MINERAL_TENURE.MTA_SITE_SP',r'WHSE_MINERAL_TENURE.MTA_ACQUIRED_TENURE_SVW']:
    ...
    # next conditional sentence is to determine of the user chose the tenure feature class
    elif os.path.basename(processedAOI) in ('MTA_SPATIAL.MTA_ACQUIRED_TENURE_SVW', 'WHSE_MINERAL_TENURE.MTA_ACQUIRED_TENURE_SVW'):
        ...
        fieldItemDict = {}
        with arcpy.da.SearchCursor(processedAOI, ["TENURE_NUMBER_ID", "TITLE_TYPE_DESCRIPTION", "ISSUE_DATE", "GOOD_TO_DATE", "OWNER_NAME", "AREA_IN_HECTARES", "SHAPE@AREA"], sqlQuery) as cursor:
            for row in cursor:
                fieldCount = 0
                ...
                for field in cursor.fields:
                    if field.title() == "Shape@Area":
                        ...
                        sheetCells(sheet, excelrow, excelcol, "Area (ha)", 10, False)
                        sheetCells(sheet, excelrow, excelcol + 1, round(row[fieldCount]/10000, 2))
                    else:
                        fieldItemDict[field] = row[fieldCount]
                        sheetCells(sheet, excelrow, excelcol, field.title(), 10, False)
                        sheetCells(sheet, excelrow, excelcol + 1, row[fieldCount])
    ...
    # Last conditional sentence is to determine of the user chose the reserves feature class
    elif os.path.basename(processedAOI) in ('MTA_SPATIAL.MTA_SITE_SVW', 'WHSE_MINERAL_TENURE.MTA_SITE_SP'):
        ...
        fieldItemDict = {}
        with arcpy.da.SearchCursor(processedAOI, ["SITE_NUMBER_ID", "RESERVE_TYPE", "MTA_SITE_ORDER_RESTR_DESC", "TOTAL_AREA", "SHAPE@AREA"], sqlQuery) as cursor:
            for row in cursor:
    ...
    for category, catList in collectFeatsCountDict.iteritems():
        ...
        for Fclass, FclassValues in catList.iteritems():
            ...
            if FclassValues[1] is not None:
                ...
                sheetCells(sheet, excelrow, excelcol + 1, Fclass, 10, False, False, False, None)
...
def createMetadataSheet(book, scratchFolder, scratchGDB):
    ...
    sheetCells(sheet, 2, 2, processedAOI)
...
def create_geomark(inFeatures, output_folder):
    ...
    # Export processed AOI feature class to shapefile
    shp = arcpy.FeatureClassToFeatureClass_conversion(inFeatures, output_folder, "AOI_geomark")
    
    # Get file extension
    fname, file_extension = os.path.splitext(str(shp))
    
    # Remove file extension period
    ...
    try:
        geomarkRequest = requests.post(geomarkEnv, files=files, headers=headers, data=fields)
        ...
        arcpy.AddMessage("    *****************************************************************")
        arcpy.AddMessage("    Error processing Geomark request for " + str(shp))
...
layerListDict, collectFeatsCountDict = processData(processedAOI, processedAOI_Hectares, IOR_Data, IORData_Fields)
...
crossReferenceDict = createInterestReportSheet(book, layerListDict, appDict)
