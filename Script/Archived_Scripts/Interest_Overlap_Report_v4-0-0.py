import arcpy, win32com.client, os, itertools, gc
...
## Keep thses parameters for tetsing purposes
#     # Input Parameters
#     #AOI = r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\projects\first_nations\Stolo (SXTA)\Sto_Lo_Traditional_Area_IOR\data\StoloTerritory\Solh Temexw.shp"
#     #AOI = r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\ArcGIS_Tools\Testing_Area\mmacrae\Test_Scripts\Interest_Overlap_Report\data\shapefiles\IHA_Dissolve.shp"
#     AOI = r"Database Connections\MEMPRD.sde\MTA_SPATIAL.MTA_SITE_SVW"
#     dbOption = 'MEMPRD'
#     #sqlQuery = '"FID" = 0'
#     sqlQuery = "SITE_NUMBER_ID = 1006556"
#     shFieldList = [str(field.name) for field in arcpy.ListFields(AOI)]
#     #output_folder = r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\Testing_Area\mmacrae\Geodatabases"
#     #output_folder = ''
#     output_excel = r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\Testing_Area\mmacrae\Test_Scripts\Interest_Overlap_Report\IOR_Test"
#     output_GDB = ''
#     output_name = "testy"
...
memprdpassword = arcpy.GetParameter(9)
bcgwpassword = arcpy.GetParameter(10)

def login(username, memprdpassword, bcgwpassword):
    ''' 
    A login prompt to get the users username and password for both MEMPRD and BCGW,
    ...
    try:
        arcpy.AddMessage("Logging into MEMPRD...")
    ...
    env.workspace = os.path.join(r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\Interim_Files", getuser(), "MEMPRD.sde")
...
def logout():
    ...
    '''
    arcpy.AddMessage("Logging out of MEMPRD and BCGW...")
    if os.path.exists(os.path.join(r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\Interim_Files", getuser())):
        rmtree(os.path.join(r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\Interim_Files", getuser()))
    else:
        pass
...
def getXLSData():
    ...
    xls = r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\Excel_Spreadsheets\InterestReport_Layer_List_MASTER.xlsx\'MineralTitles Dataset selection$'"

    xlsFields = [field.name for field in arcpy.ListFields(xls) if field.name <> "DataBC Metadata Record"]
    bufferDict = {}
    with arcpy.da.SearchCursor(xls, xlsFields) as cursor:
        for row in cursor:
            if row[1] in layerList:
                if row[11] is not None:
                    if row[1] in bufferDict:
                        bufferDict[str(row[0])].append(row[11])
                    # if not, create a new category key and add the associated Featureclass_Name value to it
                    else:
                        bufferDict[str(row[1])] = [row[11]]    
    
    return xls, xlsFields, bufferDict
...
def processData(processedAOI, processedAOI_Hectares, xls, xlsFields, bufferDict):
    ''' 
    ...
    for row in arcpy.da.SearchCursor(xls, xlsFields): 
        ...
        if row[1] in layerList:
            ...
            if row[5] == 'BCGW':
                ...
                fc = os.path.join(r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\Interim_Files", getuser(), "MEMPRD.sde", row[6])
            else:
                fc = os.path.join(row[5], row[6])
                
            arcpy.AddMessage("Processing Layer: " + row[1])
                
            for lyr, bufferDist in bufferDict.iteritems():  
                if lyr == row[1]:
                    arcpy.AddMessage("    " + "Buffering AOI for " + "'" + str(lyr) + "' layer by " + str(int(bufferDist[0])))                   
                    outBuffer = processedAOI + "_" + str(int(bufferDist[0])) + "m_buffer"
                    arcpy.Buffer_analysis(processedAOI,outBuffer, bufferDist[0])
            ...
            fieldList = [field.name for field in arcpy.ListFields(lyr) if field.name in [str(row[i]) for i in range (12, 18) if row[i] is not None]]
            ...
            if selectcount <> 0:
                ...
                if desc.shapeType == "Polygon":
                    ...
                    if row[0] in layerListDict:
                        layerListDict[row[0]].update({row[1]: str(outFC + "_clip")})
                    else:
                ...
                if row[0] in layerListDict:
                    layerListDict[row[0]].update({row[1]: "No Overlap Found"})
                else:
                    # create a new array in this slot
                    layerListDict[row[0]] = {}
                    layerListDict[row[0]].update({row[1]: "No Overlap Found"})
...
##                countClippedFeats = 0
                ...
                collectFeatsCountDict[row[1]] = countClippedFeats
    ...
    for key,value in sorted(collectFeatsCountDict.iteritems()):
        if 'Tenure' in key:
            tenureDict[key] = value
        elif 'Reserve' in key and key not in ('MINFILE - Mineral Reserve', 'Crown Reserves and Notations Tenures'):
            reserveDict[key] = value
        else:
            remainingMiningDict[key] = value
            
    arcpy.AddMessage("collectFeatsCountDict: " + str(collectFeatsCountDict))
    featsCountDict = itertools.chain(tenureDict.items(), reserveDict.items(), sorted(remainingMiningDict.items()))          
    arcpy.AddMessage("featsCountDict: " + str(featsCountDict))
    arcpy.AddMessage("layerListDict: " + str(layerListDict))
    gc.collect()
...
def createInterestReportSheet(book, layerListDict):
    ...
    sheetCells(sheet, excelrow + 2, excelcol + 1, "National Park", 10, True)
    ...
    for key1, value1 in layerListDict.iteritems():
        ...
        if key1 == 'Mineral/Coal':
            for key2, value2 in sorted(value1.iteritems()):
                if 'Tenure' in key2:
                    ...
                    hyper = '=HYPERLINK("{0}","Test")'.format(x)
    ...
    for key, value in processedlayerListDict:
        ...
        for key2, value2 in value:
            ...
            if value2 <> 'No Overlap Found':
                ...
                sheetCells(sheet, excelrow, excelcol, key2, 10, True, False, False, None)
                sheetCells(sheet, excelrow, excelcol + 1, "No Overlap Found", 10, True, False, False)
...
def createSummarySheet(book, excel, processedAOI, processedAOI_Hectares, featsCountDict):
    ...
    pic = sheet.Pictures().Insert(r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\Image\BC_EMPR_H_CMYK_pos.jpg")
    ...
    #Set Report Header Title    
    sheetCells(sheet, excelrow, excelcol, "Summary of Mineral and Coal Interests", 12, True)
...
def createDistrictSheet(book, xls, xlsFields, processedAOI):
    ...
    for row in arcpy.da.SearchCursor(xls, xlsFields):
        if row[0] == 'District':
            ...
            if row[5] == 'BCGW':
                ...
                dataSourcePath = os.path.join(r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\Interim_Files", getuser(), "MEMPRD.sde", row[6])
            else:
    ...
    for row in arcpy.da.SearchCursor(xls, xlsFields):
        if row[1] in sorted(locationList):
            ...
            # Create a select by location to test for overlap.                                
            selFeats = arcpy.SelectLayerByLocation_management("locale", "intersect", AOI)
...
def createMetadataSheet(book, scratchFolder, scratchGDB):
    ...
    sheetCells(sheet, 1, 1, "Input Feature Class for Area of Interest:", 10, True)
    sheetCells(sheet, 1, 2, AOI)
    
    sheetCells(sheet, 2, 2, "SQL Query Used on Input Feature Class:", 10, True)
    sheetCells(sheet, 2, 2, sqlQuery)
    
    sheetCells(sheet, 3, 1, "Output Folder Location of Excel Spreadsheet:", 10, True)
    sheetCells(sheet, 3, 2, output_excel) 
    
    sheetCells(sheet, 4, 1, "Configuration Excel Spreadsheet Location", 10, True)
    sheetCells(sheet, 4, 2, xls)
    
    sheet.Cells(5,1).Value = "Scratch Geodatabase Location"
    sheet.Cells(5,1).Font.Bold = True
    
    sheetCells(sheet, 5, 1, "Scratch Geodatabase Location", 10, True)
    sheetCells(sheet, 5, 2, scratchFolder + "\\" + scratchGDB)    
    
    sheet.Columns.AutoFit()
    sheet.Rows.AutoFit()

# Log into BCGW and MEMPRD Oracle databases    
login(username, memprdpassword, bcgwpassword)
...
# Set the database option for mineral titles datasets (BCGW or MEMPRD)
xls, xlsFields, bufferDict = getXLSData()

# Process layers against AOI
layerListDict, featsCountDict = processData(processedAOI, processedAOI_Hectares, xls, xlsFields, bufferDict)
