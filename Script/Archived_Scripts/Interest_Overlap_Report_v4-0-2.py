...
#     AOI = r"Database Connections\MTOPROD.sde\MTA_SPATIAL.MTA_SITE_SVW"
#     dbOption = 'MTOPROD'
#     #sqlQuery = '"FID" = 0'
...
def processData(processedAOI, processedAOI_Hectares, xls, xlsFields, bufferDict):
    ...
    for row in arcpy.da.SearchCursor(xls, xlsFields): 
        ...
        if row[1] in layerList:
            ...
            for lyr, bufferDist in bufferDict.items():
                if lyr == row[1]:
...
##                countClippedFeats = 0
    ...
    for key,value in sorted(collectFeatsCountDict.items()):
        if 'Tenure' in key:
...
def createInterestReportSheet(book, layerListDict):
    ...
    '''    
    arcpy.AddMessage(str(layerListDict))
