...
if xlsOption == "MEMPRD":
    ...
    xls = r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\Excel_Spreadsheets\InterestReport_Layer_List_MASTER_BCGW_Data.xlsx\'MineralTitles Dataset selection$'"

if scratchParam == '':
    scratchFolder = r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\ScratchGDB"
else:
...
global extentArea
arcpy.AddMessage("Opening Excel to Process Report...")
...
# Add logo to report and set size and location
pic = sheet4.Pictures().Insert(r"\\spatialfiles.bcgov\Work\em\vic\mtb\Local\MTB_Scripts\MTB_Tools\Reporting_Tools\Interest_Overlap_Report\Image\Overlap_Example.JPG")
...
if mineral_Coal_first in layerListDict:
    loop = itertools.chain([(mineral_Coal_first, layerListDict[mineral_Coal_first])],((key,valueList) for (key,valueList) in sorted(layerListDict.items()) if key != mineral_Coal_first))
else:
    loop = itertools.chain((key,valueList) for (key,valueList) in sorted(layerListDict.items()))
            ...
            if row[d["Featureclass_Name"]] == valueItem:# and row[d["Featureclass_Name"]] not in ("Archaeology Sites","Archaeology Sites within 50 metres", None):
                ...
                if countFeats == 0:
                    ...
                    if row[d["Featureclass_Name"]] in ['Tenure - Coal License Applications',
                        ...
                        for k,v in countFeatsDict.items():
                ...
                # Also, do some formatting on the cells
                    ...
                    with arcpy.da.SearchCursor(outClipFC,fieldValueList) as rows:
                        ...
                        for fieldTest in rows.fields:
                            arcpy.AddMessage("TestField = " + str(fieldTest))
